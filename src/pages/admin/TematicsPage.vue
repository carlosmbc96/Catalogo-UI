<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('07')" class="col q-gutter-sm">
        <q-btn
          class="col"
...
import FormDialog from "../../components/admin/Tematics/FormDialog.vue";
...
export default {
  ...
  setup() {
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `La temática se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} de la temática`,
          timeout: 2000,
        });
      }
    }
    ...
    async function getData(rowsPerPage, page, filter = "") {
      ...
      try {
        const response = await api.post(`/tematicas`, json);
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/tematicas/eliminar", {
              tematic: selected.value,
            });
