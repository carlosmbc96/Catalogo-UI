<template>
  <q-page class="q-pa-xl">
    <div class="row justify-between">
      <div v-if="permissionWrite('27')" class="col q-gutter-sm">
        <q-btn
          class="col"
...
import FormDialog from "../../components/admin/SellPlaces/FormDialog.vue";
  ...
  setup() {
    ...
    const columns = ref([
      ...
      },
      {
        ...
        label: "Dirección",
        align: "right",
        field: (row) => (row.direccion === null ? "" : row.direccion),
        format: (val) => `${val}`,
      },
    ]);
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `El sitio de venta se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} del sitio de venta`,
          timeout: 2000,
        });
      }
    }
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/ventas/eliminar", {
              sell: selected.value,
            });
