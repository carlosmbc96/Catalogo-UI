<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('11')" class="col q-gutter-sm">
        <q-btn
          class="col"
...
import FormDialog from "../../components/admin/Groups/FormDialog.vue";
...
export default {
  ...
  setup() {
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `El grupo se ${action_positive_txt} correctamente.`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} del grupo.`,
          timeout: 2000,
        });
      }
    }
    ...
    async function getData(rowsPerPage, page, filter = "") {
      ...
      try {
        const response = await api.post(`/grupos`, json);
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/grupos/eliminar", {
