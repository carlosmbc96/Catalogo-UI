    ...
    >
      <template #body="props">
        <tr :props="props">
          ...
          >
            {{
              props.row.fecha_nacimiento !== null &&
              props.row.fecha_nacimiento !== "null"
          ...
          >
            {{
              props.row.correo !== null && props.row.correo !== "null"
                ? props.row.correo
                : ""
            }}
          </q-td>
...
import FormDialog from "../../components/admin/Authors/FormDialog.vue";
  ...
  setup() {
    ...
    const columns = ref([
      ...
      },
      {
        ...
        align: "left",
        field: (row) =>
          row.fecha_nacimiento === null ? "" : row.fecha_nacimiento,
        format: (val) => `${val}`,
      },
      {
        name: "correo",
        ...
        align: "left",
        field: (row) => (row.correo === null ? "" : row.correo),
        format: (val) => `${val}`,
      },
    ]);
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `El autor se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} del autor`,
          timeout: 2000,
        });
      }
    }
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/autores/eliminar", {
              autor: selected.value,
            });
