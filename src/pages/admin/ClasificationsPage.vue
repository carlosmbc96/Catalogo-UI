<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('15')" class="col q-gutter-sm">
        <q-btn
          class="col"
...
import FormDialog from "../../components/admin/Clasifications/FormDialog.vue";
import confirmDelete from "../../components/admin/generals/confirmDialog.vue";
import { useCounterStore } from "../../stores/example-store";
export default {
  name: "ClasificationsPage",
  components: {
    FormDialog,
  },
  setup() {
    ...
    const columns = ref([
      {
        ...
        label: "Siglas",
        align: "left",
        field: (row) => (row.siglas === null ? "" : row.siglas),
        format: (val) => `${val}`,
      },
      {
        name: "nombre",
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `La clasificación se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} de la clasificación`,
          timeout: 2000,
        });
      }
    }
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/clasificaciones/eliminar", {
              clasification: selected.value,
            });
