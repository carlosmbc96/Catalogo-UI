<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('41')" class="col q-gutter-sm">
        <q-btn
          class="col"
    ...
    <q-table
      ...
      title="Públicos Objetivos de Ebooks"
      dense
...
import FormDialog from "../../components/admin/PublicosEbook/FormDialog.vue";
import confirmDelete from "../../components/admin/generals/confirmDialog.vue";
import { useCounterStore } from "../../stores/example-store";
export default {
  name: "PublicosEbookPage",
  components: {
    FormDialog,
  },
  setup() {
    ...
    async function getData(rowsPerPage, page, filter = "") {
      ...
      try {
        const response = await api.post(`/publicosEbook`, json);
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/publicosEbook/eliminar", {
