<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('35')" class="col q-gutter-sm">
        <q-btn
          class="col"
...
import FormDialog from "../../components/admin/Roles/FormDialog.vue";
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `El rol se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} del rol`,
          timeout: 2000,
        });
      }
    }
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/rols/eliminar", {
              rols: selected.value,
            });
