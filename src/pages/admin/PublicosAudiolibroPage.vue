<template>
  <q-page class="q-pa-xl min-table">
    <div class="row justify-between">
      <div v-if="permissionWrite('39')" class="col q-gutter-sm">
        <q-btn
          class="col"
    ...
    <q-table
      ...
      title="Públicos Objetivos de Audiolibros"
      dense
...
import FormDialog from "../../components/admin/PublicosAudiolibro/FormDialog.vue";
import confirmDelete from "../../components/admin/generals/confirmDialog.vue";
import { useCounterStore } from "../../stores/example-store";
export default {
  name: "PublicosAudiolibroPage",
  components: {
    FormDialog,
  },
  setup() {
    ...
    function notify(action, message) {
      ...
      if (message === "success") {
        $q.notify({
          ...
          caption: `El público objetivo se ${action_positive_txt} correctamente`,
          timeout: 2000,
        });
      } else {
        $q.notify({
          ...
          caption: `Ha ocurrido un error en la ${action_negative_txt} del público objetivo`,
          timeout: 2000,
        });
      }
    }
    ...
    async function getData(rowsPerPage, page, filter = "") {
      ...
      try {
        const response = await api.post(`/publicosAudiolibro`, json);
    ...
    async function deleteOperation(id) {
      $q
        ...
        .onOk(async () => {
          ...
          try {
            const response = await api.post("/publicosAudiolibro/eliminar", {
              publico: selected.value,
            });
