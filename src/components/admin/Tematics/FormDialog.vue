        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Temática" : "Crear Temática" }}
            </div>
            <div v-else class="row justify-center">Detalles de la Temática</div>
            <q-btn
              dense
              flat
              round
...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/tematicas/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/tematicas/${act}`, form.value);
          if (typeof response.data === "string") {
