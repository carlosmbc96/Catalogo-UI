        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Cartulina" : "Crear Cartulina" }}
            </div>
            <div v-else class="row justify-center">
              Detalles de la Cartulina
            </div>
            <q-btn
              dense
              flat
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe una cartulina de este tipo";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/cartulinas/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/cartulinas/${act}`, form.value);
          if (typeof response.data === "string") {
