        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Clasificador" : "Crear Clasificador" }}
            </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe un clasificador con este nombre";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/clasificadores/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/clasificadores/${act}`, form.value);
          if (typeof response.data === "string") {
