        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Idioma" : "Crear Idioma" }}
            </div>
            <div v-else class="row justify-center">Detalles del Idioma</div>
            <q-btn
              dense
              flat
              round
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe este idioma";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/idiomas/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/idiomas/${act}`, form.value);
          if (typeof response.data === "string") {
