        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Grupo" : "Crear Grupo" }}
            </div>
            <div v-else class="row justify-center">Detalles del Grupo</div>
            <q-btn
              dense
              flat
              round
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe un grupo con este nombre";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/grupos/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/grupos/${act}`, form.value);
          if (typeof response.data === "string") {
