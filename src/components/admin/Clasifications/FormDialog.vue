        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Clasificación" : "Crear Clasificación" }}
            </div>
        ...
        <q-card-section>
          <div class="row items-center q-mb-xs" style="align-items: baseline">
            <div class="col-12 col-md">
              <span v-if="!readonly_show" class="require">*</span> Siglas:
            </div>
            <div class="col-12 col-md-7">
              <q-input
                ...
                @update:model-value="
                  (v) => {
                    form.numero = v.replace(/[^0-9-a-zA-Z]/g, '');
                  }
                "
                color="grey-14"
                outlined
...
export default {
  ...
  watch: {
    ...
    item() {
      if (this.item && this.item.id) {
        ...
        old_siglas.value = this.item.siglas;
        old_nombre.value = this.item.nombre;
        form.value.siglas = this.item.siglas;
  ...
  setup(props, { emit }) {
    ...
    onBeforeMount(async () => {
      ...
      try {
        const response = await api.post(`/clasificaciones`, json);
    ...
    function exist() {
      let found = false;
      if (old_siglas.value.toLowerCase() !== form.value.siglas.toLowerCase()) {
        let i = 0;
        while (!found && i < all_items.value.length) {
          if (
            all_items.value[i].siglas.toLowerCase() ==
            form.value.siglas.toLowerCase()
          ) {
            found = true;
          }
          i++;
        }
        return !found || "Estas siglas ya existen";
      }
    }
    ...
    function existNombre() {
      ...
      return !found || "Ya existe una clasificación con este nombre";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/clasificaciones/${act}`, form.value);
    ...
    function disable() {
      return form.value.nombre && form.value.siglas;
    }
    ...
    function confirmChanges() {
      if (form.value.id) {
        if (
          ...
          form.value.siglas !== item_ref.value.siglas
        ) {
          show_confirm.value = true;
        } else clearForm("cancelar");
      } else if (form.value.nombre || form.value.siglas) {
        show_confirm.value = true;
      } else clearForm("cancelar");
    }
    return {
