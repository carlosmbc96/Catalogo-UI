<template>
  <q-dialog v-model="show" persistent>
    <q-card class="q-pa-md dialog-form" style="width: 45vw">
      <q-form
        ref="form_ref"
        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{
                form.id
                  ? "Editar Público Objetivo de Ebooks"
                  : "Crear Público Objetivo de Ebooks"
              }}
            </div>
            <div v-else class="row justify-center">
              Detalles del Público Objetivo de Ebooks
            </div>
            <q-btn
              dense
              flat
          ...
          <div class="row items-center q-mb-xs" style="align-items: baseline">
            ...
            <div class="col-12 col-md-7">
              <q-input
                ...
                @update:model-value="
                  (v) => {
                    form.nombre = v.replace(/[^0-9-a-zA-ZÁÉÍÓÚáéíóú ]/g, '');
                  }
                "
                outlined
                color="grey-14"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function exist() {
      ...
      return !found || "Este público ya existe";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/publicosEbook/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/publicosEbook/${act}`, form.value);
          if (typeof response.data === "string") {
