        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Papel" : "Crear Papel" }}
            </div>
            <div v-else class="row justify-center">Detalles del Papel</div>
            <q-btn
              dense
              flat
              round
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe un papel de este tipo";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/papeles/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/papeles/${act}`, form.value);
          if (typeof response.data === "string") {
