        ...
        <q-card-section class="text-h6 text-bold">
          <div class="row items-center justify-between">
            <div v-if="!readonly_show" class="row justify-center">
              {{ form.id ? "Editar Tipo de Libro" : "Crear Tipo de Libro" }}
            </div>
            <div v-else class="row justify-center">
              Detalles del Tipo de Libro
            </div>
            <q-btn
              dense
              flat
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function existNombre() {
      ...
      return !found || "Ya existe un tipo de libro con este nombre";
    }
    ...
    async function action(act) {
      ...
      if (act === "editar") {
        try {
          const response = await api.put(`/tiposLibro/${act}`, form.value);
        ...
        try {
          const response = await api.post(`/tiposLibro/${act}`, form.value);
          if (typeof response.data === "string") {
